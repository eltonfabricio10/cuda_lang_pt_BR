...
class Setting:
    ...
    meta_info = [
        {"opt": "task_bullet_open",
         "cmt": [_("Character of opened task, one of:"),
                 " - | ❍ | ❑ | ■ | □ | ☐ | ▪ | ▫ | – | — | ≡ | → | › | [ ]"],
         "def": "☐",
         "frm": "strs",
         "lst": ["-", "❍", "❑", "■", "□", "☐", "▪", "▫", "–", "—", "≡", "→", "›", "[ ]"]
        ...
        {"opt": "task_bullet_done",
         "cmt": [_("Character of completed task, one of:"),
                 " + | ✓ | ✔ | √ | [x]"],
         "def": "✔",
         "frm": "strs",
         "lst": ["+", "✓", "✔", "√", "[x]"]
        ...
        {"opt": "task_bullet_cancel",
         "cmt": [_("Character of cancelled task, one of:"),
                 " x | ✘ | [-]"],
         "def": "✘",
         "frm": "strs",
         "lst": ["x", "✘", "[-]"]
        ...
        {"opt": "date_format",
         "cmt": [_("Format of date/time. Possible macro characters are:"
        ...
        {"opt": "archive_name",
         "cmt": [_("Caption which is inserted above archived tasks")],
         "def": "Archive:",
         "frm": "str",
         },
        {"opt": "add_created_tag",
         "cmt": [_("When creating new task, add tag @created")],
         "def": True,
         "frm": "bool",
         },
        {"opt": "done_tags",
         "cmt": [_("When task is changed between Done/Cancelled, add tag @done/@canceled")],
         "def": True,
         "frm": "bool",
         },
        {"opt": "done_date",
         "cmt": [_("When task is changed between Done/Cancelled, add timestamp")],
         "def": True,
         "frm": "bool",
         }
    ]
    ...
    def get_cfg(self):
        with open(config_file, encoding='utf8') as rf:
            ...
            if self.done_tags:
                self.done_tag = TAGS.get("done")
                self.cancel_tag = TAGS.get("cancelled")
            else:
                self.done_tag = ""
                self.cancel_tag = ""
