...
#        return s
    ...
    def memo_update(self):
        ...
        #dlg_proc(self.h_dlg, DLG_CTL_PROP_SET, name='header', prop={
            #'cap': '{} markers'.format(len(self.memo.attr(MARKERS_GET_DICT)))
        #})
    ...
    def apply_url_markers(self, tag='', info=''):
        url_markers = [m for m in self.memo.attr(MARKERS_GET_DICT) if m['tag'] == -100] # url markers have tag -100
        for m in url_markers:
            for x in range(m['x'], m['x']+m['len']): # sadly only non-multiline URLs will be colored/underlined
                self.memo.attr(
                    MARKERS_ADD,
                    tag = 0,
                    x = x,
                    y = m['y'],
                    len = 1,
                    color_font = m['color_font'],
                    color_bg = m['color_bg'],
                    color_border = m['color_border'],
                    border_down = m['border_down']
                    )
