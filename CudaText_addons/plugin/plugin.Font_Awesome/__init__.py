...
class Command:
    ...
    def callback_listbox_drawitem(self, id_dlg, id_ctl, data='', info=''):
        ...
        item = listbox_proc(self.h_list, LISTBOX_GET_ITEM_PROP, index=index)['text'].split('|')
        name = item[0]
        hex = item[1]
    ...
    def get_icon(self, idd, idc, data):
        ...
        item = listbox_proc(self.h_list, LISTBOX_GET_ITEM_PROP, index=index_sel)['text'].split('|')
